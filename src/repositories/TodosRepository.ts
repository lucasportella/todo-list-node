...
export class TodosRepository {
  ...
  async findOne(userId: number, id: number): Promise<Todo | null> {
  ...
  async createTodo(newTodo: NewTodo): Promise<Todo | null> {
    ...
    const todo = await this.findOne(newTodo.userId, result.insertId)
    return todo;
  }
  ...
  async updateTodo(todo: UpdateTodo): Promise<Todo | null> {
    ...
    const updatedTodo = await this.findOne(todo.userId, todo.id);
